		...
		switch ($reporte){
			case "Prenda":
				...
				if (COUNT($res)>0)
				{
					...
					$this->excel->getActiveSheet()->setTitle('Cancelaciones de Hipotecas');
					$this->excel->setActiveSheetIndex(0)->mergeCells('A1:M1');
					...
					$this->excel->getActiveSheet()->getStyle('A1:M1')->applyFromArray($estiloTituloReporte);
					$this->excel->setActiveSheetIndex(0)->mergeCells('A1:M1');
					...
					$this->excel->getActiveSheet()->getStyle("J{$contador}")->getFill()->getStartColor()->setRGB('FFCCFF');
					$this->excel->getActiveSheet()->getStyle("K{$contador}")->getFill()->getStartColor()->setRGB('FFCCFF');
					$this->excel->getActiveSheet()->getStyle("L{$contador}")->getFill()->getStartColor()->setRGB('FFCCFF');
					$this->excel->getActiveSheet()->getStyle("M{$contador}")->getFill()->getStartColor()->setRGB('FFCCFF');
					...
					$this->excel->getActiveSheet()->setCellValue("M{$contador}", 'FECHA CREACION');
					...
					foreach($res as $d){
						...
						$this->excel->getActiveSheet()->setCellValue("L{$contador}", $d->finalizacionTramite);
						$this->excel->getActiveSheet()->setCellValue("M{$contador}", $d->fechacrea);
					}
					$estiloTituloReporte = array(
						'font' => array(
							'name'      => 'Arial',
					...
					$this->excel->getActiveSheet()->getStyle('A2:M2')->applyFromArray($estiloTituloReporte);
			...
			case "Hipoteca":
				...
				if (COUNT($res)>0)
				{
					...
					$this->excel->getActiveSheet()->setCellValue("K{$contador}", 'Observaciones');
					$this->excel->getActiveSheet()->setCellValue("L{$contador}", 'Fecha Creacion');
					...
					foreach($res as $d){
						...
						$this->excel->getActiveSheet()->setCellValue("K{$contador}", $d->Observaciones);
						$this->excel->getActiveSheet()->setCellValue("L{$contador}", $d->fechacrea);
			...
			case "0":
				...
				if (COUNT($res)>0)
				{
					...
					$this->excel->setActiveSheetIndex(0);
					$this->excel->getActiveSheet()->setCellValue("A1", 'Cancelaciones de Prenda: '.$fechainicio.' al '.$fechafin);
					$this->excel->getActiveSheet()->setTitle('Cancelaciones de Prenda');
					$this->excel->setActiveSheetIndex(0)->mergeCells('A1:M1');
				...
				if (COUNT($res)>0)
				{
					...
					$this->load->library('excel'); $this->excel->setActiveSheetIndex(0);
					$this->excel->setActiveSheetIndex(0);
					$this->excel->getActiveSheet()->setCellValue("A".$contador, 'Cancelaciones de Hipotecas: '.$fechainicio.' al '.$fechafin);
					$this->excel->getActiveSheet()->setTitle('Cancelaciones de Hipotecas');
					...
					$this->excel->getDefaultStyle()->applyFromArray($style);
					$this->excel->getActiveSheet()->getStyle('A'.$contador.':L'.$contador)->applyFromArray($estiloTituloReporte);
