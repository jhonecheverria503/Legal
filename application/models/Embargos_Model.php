...
class Embargos_Model extends CI_Model
{
	...
	public function saveCliente($data){
		$this->db->insert('Bienes_Embargados',$data);
	...
	public function ObtenerCliente($id){
		$this->db->select("p.id,p.Nombre,p.BienEmbargado,p.Estado,p.Monto,p.Placa,p.Observaciones,p.Fecha");
		$this->db->from("Bienes_Embargados AS p");
		$this->db->where("id",$id);
		$query=$this->db->get();
		return $query->result();
	}
	...
	public function actualizarCliente($data,$where){
		$this->db->update('Bienes_Embargados',$data,$where);
		$res=$this->db->affected_rows();
		$mensaje = array();
		if ($res){
			$mensaje['estado']=true;
